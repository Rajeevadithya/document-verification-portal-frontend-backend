...
import { CheckCircle, Download, Edit, ExternalLink, Eye, FileUp, LoaderCircle, Trash2, Upload, XCircle } from "lucide-react";
...
export function InvoiceModule() {
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<string>(searchParams.get("doc") || "");
  ...
  const loadData = async (preferredInvoice?: string) => {
    ...
    try {
      ...
      const nextInvoice = preferredInvoice || selectedInvoice || helpers[0]?.id || typedInvoices[0]?.invoice_number || "";
      if (nextInvoice) {
  ...
  useEffect(() => { void loadData(searchParams.get("doc") || undefined); }, [searchParams]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <FilterBar docType="INV" onSearch={(next) => { setFilters(next); if (next.docNumber) void selectInvoice(next.docNumber); }} valueHelpItems={valueHelpItems} />

      <div className="flex-1 overflow-auto p-4 flex flex-col gap-4">
        {(error || infoMessage) ? <div className="border px-4 py-3" style={{ borderColor: error ? "#F0B2B2" : "#B7E0C1", backgroundColor: error ? "#FBEAEA" : "#EEF5EC", color: error ? "#BB0000" : "#107E3E", borderRadius: "2px", fontSize: "12px" }}>{error || infoMessage}</div> : null}

        <SectionTable title="Invoice Overview" badge={`${invoiceRows.length} invoice(s)`}>
          <div className="overflow-x-auto">
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              <thead><tr style={{ backgroundColor: "#f5f5f5", borderBottom: "1px solid #d9d9d9" }}>{["Invoice Number", "PR Number", "PO Number", "GRN Number", "Status", "Created"].map((header) => <th key={header} className="text-left" style={TH_STYLE}>{header}</th>)}</tr></thead>
              <tbody>
                {invoiceRows.length === 0 ? <tr><td colSpan={6} style={{ padding: "20px", textAlign: "center", fontSize: "12px", color: "#8a8b8c" }}>No invoices found.</td></tr> : invoiceRows.map((row, index) => {
                  ...
                  return (
                    <tr key={row.invoiceNumber} onClick={() => void selectInvoice(row.invoiceNumber)} style={{ borderBottom: "1px solid #eeeeee", backgroundColor: selectedInvoice === row.invoiceNumber ? "#EAF1FF" : index % 2 === 0 ? "#ffffff" : "#fafafa", cursor: "pointer" }}>
                      ...
                      <td style={TD_STYLE}><span style={{ fontSize: "11px", color: tone.color, backgroundColor: tone.bg, padding: "2px 6px", borderRadius: "2px", fontWeight: "600" }}>{row.status}</span></td>
        ...
        {subTab === "upload" && (
          <div className="flex gap-4 flex-wrap">
            ...
            <SectionTable title="OCR Validation">
              <div className="p-4 flex flex-col gap-3" style={{ fontSize: "12px", color: "#32363a" }}>
                {!lastValidation ? <span style={{ color: "#8a8b8c" }}>Upload or replace a document to see the latest OCR validation result.</span> : <><div className="flex items-center gap-2">{lastValidation.ocr_status === "VALID" ? <CheckCircle size={16} color="#107E3E" /> : <XCircle size={16} color="#E9730C" />}<span style={{ fontWeight: "600", color: statusTone(lastValidation.ocr_status).color }}>{lastValidation.ocr_status}</span></div><div style={{ backgroundColor: "#f7f7f7", border: "1px solid #e5e5e5", padding: "10px", borderRadius: "2px" }}>{lastValidation.ocr_rejection_detail?.summary || "All OCR checks passed."}</div></>}
              </div>
            </SectionTable>
          </div>
        )}
          ...
          <SectionTable title={subTab === "change" ? "Manage Invoice Document" : "Uploaded Invoice Document"} badge={`${currentDocuments.length} document(s)`}>
            <div className="overflow-x-auto">
              <table className="w-full" style={{ borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {currentDocuments.length === 0 ? <tr><td colSpan={7} style={{ padding: "20px", textAlign: "center", fontSize: "12px", color: "#8a8b8c" }}>No invoice document found.</td></tr> : currentDocuments.map((document) => (
